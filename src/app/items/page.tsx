...
import { useCNFT } from "@/context/cnftCollection.context";
import { useWallet } from "@solana/wallet-adapter-react";
import { motion } from "framer-motion";
import { useRouter } from "next/navigation";

const Page = () => {
  const { cnfts, error, isNftLoaded } = useCNFT();
  ...
  if (isNftLoaded === "loaded" && cnfts) {
    return (
      <main className="flex min-h-screen flex-col items-center justify-center px-4 sm:p-24">
        ...
        >
          {cnfts.length} Chronicle Items NFTs Detected!
        </motion.h1>
        <motion.h2
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-10 mt-6 sm:mt-10">
          {cnfts.map((nft) => (
            <div className="group" key={nft.id}>
